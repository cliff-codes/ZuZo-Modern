...
import { ChevronRight, Star, Quote, Calendar } from "lucide-react";
          ...
          <div className="container mx-auto px-4 lg:px-8">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-lg lg:text-xl text-muted-foreground">
                Discover real results from companies that have partnered with ZuZo to reduce costs, scale operations, and deliver exceptional customer experiences.
              </p>
            </div>
          </div>
        </section>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                {filteredCaseStudies.map((caseStudy) => (
                  <Link key={caseStudy.id} href={`/resources/case-studies/${caseStudy.slug}`}>
                    <Card className="h-full hover-elevate active-elevate-2 cursor-pointer overflow-hidden" data-testid={`case-study-card-${caseStudy.slug}`}>
                      {caseStudy.imageUrl && (
                      ...
                      <CardHeader className="gap-4">
                        <div className="flex items-center gap-2 flex-wrap">
                          {caseStudy.industry && (
                            <Badge variant="secondary" data-testid={`badge-industry-${caseStudy.slug}`}>
                              {caseStudy.industry}
                            </Badge>
                          )}
                        </div>
                        <h3 className="font-heading font-semibold text-2xl" data-testid={`title-${caseStudy.slug}`}>
                          {caseStudy.title}
                        </h3>
        ...
        <section className="py-16 lg:py-20">
          <div className="container mx-auto px-4 lg:px-8">
            <div className="max-w-3xl mx-auto text-center mb-12">
              ...
              <p className="text-lg text-muted-foreground">
                Hear directly from businesses that have transformed their operations with ZuZo
              </p>
            </div>
            ...
            {testimonialsLoading ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {testimonials.map((testimonial) => (
                  <Card key={testimonial.id} className="hover-elevate" data-testid={`testimonial-card-${testimonial.id}`}>
                    <CardContent className="p-6">
